...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\04Ben\\OneDrive\\Desktop\\Sternberg\\Sternberg.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # create an object to store info about Routine Trial_Test
    Trial_Test = data.Routine(
        name='Trial_Test',
        components=[],
    )
    Trial_Test.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    # store start times for Trial_Test
    Trial_Test.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    Trial_Test.tStart = globalClock.getTime(format='float')
    Trial_Test.status = STARTED
    thisExp.addData('Trial_Test.started', Trial_Test.tStart)
    Trial_Test.maxDuration = None
    # keep track of which components have finished
    Trial_TestComponents = Trial_Test.components
    for thisComponent in Trial_Test.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    Trial_Test.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            Trial_Test.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in Trial_Test.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    for thisComponent in Trial_Test.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for Trial_Test
    Trial_Test.tStop = globalClock.getTime(format='float')
    Trial_Test.tStopRefresh = tThisFlipGlobal
    thisExp.addData('Trial_Test.stopped', Trial_Test.tStop)
    thisExp.nextEntry()
